...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.text((10, 100), "HippoVolume.AI", (255, 255, 255), font=header_font)
    draw.multiline_text((10, 150),
                        f"Patient ID: {header.PatientID}\nAccession Number: {header.AccessionNumber}\n\nAnterior Vol: {inference['anterior']}\nPosterior Vol: {inference['posterior']}\nTotal Vol: {inference['total']}\n\nShowing Prediction at axial slices [{slice_nums[0]}, {slice_nums[1]}, {slice_nums[2]}]",
    ...
    for slice_num in slice_nums:
        ...
        slicePred = pred_vol[:, :, slice_num]
        slicePred[slicePred > 1] = 1
        ...
        nd_imgPred = np.flip((slicePred/np.max(slicePred))*0xff).T.astype(np.uint8)
        # This is how you create a PIL image from numpy array
        newSize = (slice.shape[0]*8, slice.shape[1]*8)
        pil_i = Image.fromarray(nd_img, mode="L").convert("RGBA").resize(newSize)
        pil_iPred = Image.fromarray(nd_imgPred, mode="L").convert("RGBA").resize(newSize)
        pilBlended = Image.blend(pil_i, pil_iPred, 0.5)
        # Paste the PIL image into our main report image object (pimg)
        pimg.paste(pilBlended, box=(10+i, 350))
